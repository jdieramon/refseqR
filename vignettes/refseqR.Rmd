...
To install the packages you'll need to do the following:

```r
if (!require("BiocManager")) {
  install.packages("BiocManager")
}
BiocManager::install("refseqR")
```
...
extract_from_xm <- function(listName, feat = "tissue") {
    ...
    if (!feat %in% toMatch) {
        stop("Allowed features: 'sex','genotype, 'tissue'")
    } 
    ...
    for (i in seq(listName[[1]])) {
        ...
        if (length(grep(pattern, val, value = TRUE)) > 0) {
...
cds <- entrez_fetch(
    db = "nuccore", id = xm, rettype = "fasta",
...
extract_mol.wt_from_xp <- function(listName, feat = "calculated_mol_wt") {
    ...
    # if the loop could not find the 'feat', return 0
